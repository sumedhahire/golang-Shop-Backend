...
func (tx *Tx) init() {
	tx.TblAuthClient = NewTblAuthClientClient(tx.config)
	tx.TblAuthToken = NewTblAuthTokenClient(tx.config)
	tx.TblCart = NewTblCartClient(tx.config)
	tx.TblInventory = NewTblInventoryClient(tx.config)
	tx.TblInventoryTag = NewTblInventoryTagClient(tx.config)
	tx.TblPayment = NewTblPaymentClient(tx.config)
	tx.TblTag = NewTblTagClient(tx.config)
	tx.TblUser = NewTblUserClient(tx.config)
}
...
// applies a query, for example: TblAuthClient.QueryXXX(), the query will be executed
